// Imperative C++ program to count the number of times an integer occurs in a linked list
...
void push(node** headRef, int dataToPush){ // pass in head reference i.e. address of the head, and the data to be pushed onto the list
	node* newNode = new node;    // create a new linked list node using new which dynamically allocates the number of bytes needed on the heap and returns a pointer
...
void deallocateMemory(node** headRef){ // pass in head reference i.e. address of the head, so that its contents may be altered 
	node* currentNode = *headRef;      // pointer to the first node in the list
	node* temp;                        // pointer to store the address of the next node while current is being deallocated
	while (currentNode != NULL){         // traverse the linked list
		...
		delete temp;                      // deallocate the heap memory of the previous node
	}
...
int count_while(node* head, int numberToSearchFor){ // pass in the pointer to the head of the list, and the int to search for
	node* currentNodePtr = head; // make a local copy of the head pointer as it is a stack variable and should remain unchanged
...
int count_for(node* head, int numberToSearchFor){
	node* currentNodePtr;
	int count = 0;
...
int main(){
	node* head = NULL;  // list is currently empty, thus initialize a NULL pointer as its head

// OOP C++ program to pop the top node off of a linked list
...
int linkedList::pop(){
	node* nodeToPop = this->head;  // get address of the node to be popped i.e. the first node in the list
	int dataToPop = nodeToPop->data;  // retreive data that is to be popped
	this->head = nodeToPop->next;  // assign address of second node in list to head
	delete nodeToPop;              // deallocate heap memory of the first node 
...
int main(){
	...
	cout << "popped value is " << myList->pop() <<  endl;
	cout << "node-0 is " << myList->getNth(0) <<  endl;
	cout << "node-1 is " << myList->getNth(1) <<  endl;
	cout << "node-2 is " << myList->getNth(2) <<  endl;
	cout << "node-3 is " << myList->getNth(3) <<  endl;

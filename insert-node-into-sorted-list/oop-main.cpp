...
class linkedList{
	public:
		...
		void insertIntoSortedList(int dataToInsert);		
};
...
void linkedList::insertIntoSortedList(int dataToInsert){

	node* current = this->head;  // create a local variable to store the contents of head i.e. address of first node

	if(current == NULL || dataToInsert <= current->data)  // edge cases, if list is empty, or data to be inserted is smallest value
		push(&(this->head), dataToInsert);  // push onto the head of the list
	else {

// C program to sort an unsorted list
...
int main(){
	...
	push(&head, 33);         // pass in the address of the head pointer, and the value to push onto the list
	push(&head, 21);          // must pass in the address of the head pointer (reference pointer) so that the callee can alter
	push(&head, 101);          // the caller's memory, since the head pointer will change with each push (onto the top of the list)
	push(&head, 96);          // otherwise sending the head pointer itself will pass by value creating a local copy in the push function
